  ...
  <div class="alert-center-page">
    ...
    <div class="alert-header">
      ...
      <div class="header-actions">
        <el-button @click="$emit('close')">💬 智能问数</el-button>
      </div>
    </div>
    ...
    <el-card class="alert-list-section">
      ...
      <el-table :data="filteredAlerts" style="width: 100%">
        ...
        <el-table-column label="操作" width="150">
          <template #default="{ row }">
            <el-button size="small" type="primary" @click="handleAcknowledge(row)">确认</el-button>
...
const filteredAlerts = computed(() => {
  ...
  return alerts.value.filter(alert => alert.level.includes(getLevelFilter(filterType.value)))
})
...
const handleAcknowledge = (row) => {
  alert(`确认预警：${row.title}`)
}

const handleAssign = (row) => {
  alert(`分配预警：${row.title}`)
}
